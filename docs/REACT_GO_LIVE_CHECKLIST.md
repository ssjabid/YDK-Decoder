...
- [ ] **1d** **DuelingBook extraction (only if you plan to extract, not hand-build):** run the Chrome extension on a replay as you used to, then open the **React Combos tab** — does the extracted combo appear? **This is the one thing I haven't been able to prove for your setup** (combos are stored per-origin/port, and the extension may write to a different one than the React app). **If it doesn't appear, tell me your exact URLs and I'll wire the handoff** — without touching `combo-import-helper.js`. Fallback meanwhile: hand-build (1a) or Settings → Restore from a backup.
